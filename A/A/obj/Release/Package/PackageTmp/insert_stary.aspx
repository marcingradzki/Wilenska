...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <asp:GridView ID="GridView3" runat="server" AutoGenerateColumns="False" CellPadding="4" DataKeyNames="ID" DataSourceID="opłaty" ForeColor="#333333" GridLines="None" class="table table-hover">
        ...
        <Columns>
            ...
            <asp:BoundField DataField="Prąd" HeaderText="Prąd" SortExpression="Prąd" />
            <asp:BoundField DataField="Gaz" HeaderText="Gaz" SortExpression="Gaz" />
            <asp:BoundField DataField="Czynsz" HeaderText="Czynsz" SortExpression="Czynsz" />
            <asp:BoundField DataField="Internet" HeaderText="Internet" SortExpression="Internet" />
            <asp:BoundField DataField="Składka" HeaderText="Składka" SortExpression="Składka" />
            <asp:BoundField DataField="Suma_kosztów" HeaderText="Suma_kosztów" SortExpression="Suma_kosztów" />
            <asp:BoundField DataField="Helper" HeaderText="Helper" SortExpression="Helper" />
        </Columns>
        <EditRowStyle BackColor="#2461BF" />
        <FooterStyle BackColor="#507CD1" Font-Bold="True" ForeColor="White" />
        <HeaderStyle BackColor="#507CD1" Font-Bold="True" ForeColor="White" />
        ...
        <asp:SqlDataSource ID="opłaty" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" SelectCommand="
            UPDATE `dbo`.`Opłaty` 
                SET `dbo`.`Opłaty`.`Woda` = (12.34 + ((Media.`Woda_zimna` + Media.`Woda_ciepła`)*7.88) + (13.09 * Media.`Woda_ciepła`)) * Opłaty.Helper ,
                ...
                UPDATE `dbo`.`Opłaty` 
                SET `dbo`.`Opłaty`.`Suma_kosztów` = (Opłaty.Woda + Opłaty.Prąd + Opłaty.Gaz +Opłaty.Czynsz + Opłaty.Internet + Opłaty.Składka)
                FROM .`dbo`.`Media`
   ...
   <!-- <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DBConnectionString %>" SelectCommand="
            UPDATE [dbo].[Opłaty] 
                SET [dbo].[Opłaty].[Woda] = (12.34 + ((Media.[Woda_zimna] + Media.[Woda_ciepła])*7.88) + (13.09 * Media.[Woda_ciepła])) * Opłaty.Helper ,
